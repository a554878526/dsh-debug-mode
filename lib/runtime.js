...
import { appendDebugModeState, foldDebugModeState } from "./state.js";
...
function lastDebugModeStateEvent(session) {
...
function setupToolState(agent, stateSeq, states) {
...
function observeSuccessfulProbeWrite(exec, result, states) {
    const agent = exec.agent;
    if (agent === undefined || result.isError)
        return;
    const stateEvent = lastDebugModeStateEvent(agent.session);
    if (stateEvent?.data.phase !== 'setup' && stateEvent?.data.phase !== 'analyzing')
        return;
    ...
    if (command !== undefined) {
        if (command.includes('new_debug_session.py')) {
            ...
            if (transport !== undefined)
                states.set(agent.id, { ...state, transport, ingestStarted: false });
            return;
        }
...
async function* enforceSetupResponse(options, next, sessions, setupStates) {
...
export function installDebugModeRuntime(ctx) {
    ...
    ctx.on('agent/pre-step', async ({ agent, messages }, next) => {
        ...
        if (state.phase !== 'waiting-for-repro')
            return next();
        if (human === undefined)
            return { kind: 'reject' };
    ...
    ctx.tools.guard((exec) => {
        ...
        if (agent === undefined)
            return undefined;
        const stateEvent = lastDebugModeStateEvent(agent.session);
        if (stateEvent?.data.phase === 'waiting-for-repro')
    ...
    ctx.on('llm/stream', (options, next) => enforceSetupResponse(options, next, ctx.sessions, setupToolStates), { global: true });
}
