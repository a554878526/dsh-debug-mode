...
const PHASES = new Set<string>(['setup', 'waiting-for-repro', 'analyzing', 'inactive'])

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === 'object' && value !== null && !Array.isArray(value)
}

function validatePayload(data: unknown, fail: InvariantFailure): void {
  if (!isRecord(data)) return fail('debug-mode/state data must be an object')
  if (data['version'] !== 1) fail(`debug-mode/state uses unsupported version ${String(data['version'])}`)
  ...
  if (phase !== 'waiting-for-repro') {
    if (handoff !== undefined) fail(`debug-mode/state phase ${phase} must not carry a handoff`)
    return
  }
...
function validateStateEvent(
  events: readonly SessionEvent[],
  event: Extract<SessionEvent, { type: 'debug-mode/state' }>,
  fail: InvariantFailure,
): void {
  validatePayload(event.data, fail)
  let phase: DebugModePhase = 'inactive'
  let phaseIndex = -1
  ...
  if (event.data.phase === 'setup') return
  if (event.data.phase === 'waiting-for-repro') {
    ...
    if (handoff === undefined) fail('debug-mode/state waiting-for-repro requires a preceding validated handoff chunk')
...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  ...
  ctx.on('internal/dispatch', (_mode, eventName, args) => {
    ...
    if (event.type === 'debug-mode/state') validateStateEvent(session.events, event, fail)
  }, { global: true })
}, { inject: ['sessions'] })
